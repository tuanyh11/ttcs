const product = {
  data: {
    products: {
      edges: [
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
                {
                  node: {
                    databaseId: 137,
                    count: null,
                    name: "Brake Discs",
                    slug: "brake-discs",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyNDI=",
            name: "Carbon Ceramic Brake Kit",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 242,
            productId: 242,
            regularPrice: "$80.00",
            salePrice: null,
            slug: "carbon-ceramic-brake-kit",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyNDA=",
            name: "Engine Piston NH220",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 240,
            productId: 240,
            regularPrice: "$150.00",
            salePrice: "$120.00",
            slug: "engine-piston",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMzc=",
            name: "Engine Crankshaft 2.5 D 4wd",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 237,
            productId: 237,
            regularPrice: "$290.00",
            salePrice: null,
            slug: "engine-crankshaft",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMzU=",
            name: "Shock Absorber",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 235,
            productId: 235,
            regularPrice: "$570.00",
            salePrice: "$550.00",
            slug: "shock-absorber",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMjE=",
            name: "V6 Engine Piston",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 221,
            productId: 221,
            regularPrice: "$570.00",
            salePrice: "$550.00",
            slug: "v6-engine-piston",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMTc=",
            name: "Winter Royal Black Tire",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 217,
            productId: 217,
            regularPrice: "$80.00",
            salePrice: null,
            slug: "winter-royal-black-tire",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMTU=",
            name: "Spherical Roller Bearing",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 215,
            productId: 215,
            regularPrice: "$290.00",
            salePrice: null,
            slug: "spherical-roller-bearing",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMTM=",
            name: "DOE Replica Wheels",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 213,
            productId: 213,
            regularPrice: "$330.00",
            salePrice: "$270.00",
            slug: "doe-replica-wheels",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoyMDA=",
            name: "Heater Control Valve",
            averageRating: 0,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 200,
            productId: 200,
            regularPrice: "$190.00",
            salePrice: "$170.00",
            slug: "heater-control-valve",
          },
        },
        {
          node: {
            commentCount: 1,
            productCategories: {
              edges: [
                {
                  node: {
                    id: "dGVybTozNA==",
                    databaseId: 34,
                    slug: "tire_part",
                    name: "Tire Parts",
                  },
                },
              ],
            },
            act_product: {
              table: [
                {
                  row: [
                    {
                      data: "<p>Audi</p>\n",
                      fieldGroupName: "row",
                      head: "Make",
                    },
                    {
                      data: "<p>A5</p>\n",
                      fieldGroupName: "row",
                      head: "Model",
                    },
                    {
                      data: "<p>2017</p>\n",
                      fieldGroupName: "row",
                      head: "Year",
                    },
                    {
                      data: "<p>Sedan</p>\n",
                      fieldGroupName: "row",
                      head: "Body Style",
                    },
                  ],
                },
              ],
              fieldGroupName: "act_product",
            },
            comments: {
              nodes: [
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 3,
                  databaseId: 3,
                  id: "Y29tbWVudDoz",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:41",
                  content:
                    "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                  replies: {
                    nodes: [],
                  },
                },
                {
                  rating: Math.floor(Math.random() * 5),

                  commentId: 2,
                  databaseId: 2,
                  id: "Y29tbWVudDoy",
                  author: {
                    node: {
                      avatar: {
                        url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                      },
                      email: null,
                      id: "dXNlcjox",
                      name: "chakta",
                    },
                  },
                  date: "2023-02-20 07:48:25",
                  content:
                    "<p>But I must explain to you how all this mistaken idea of denouncing pleasure and praising pawas born and I will give you a complete account</p>\n",
                  replies: {
                    nodes: [
                      {
                        author: {
                          node: {
                            avatar: {
                              url: "http://0.gravatar.com/avatar/92c2a5ef5dcbd564ed3bf66c4d978022?s=96&d=mm&r=g",
                            },
                            email: null,
                            id: "dXNlcjox",
                            databaseId: 1,
                            url: null,
                            name: "leo",
                          },
                        },
                        rating: Math.floor(Math.random() * 5),
                        commentId: 3,
                        id: "Y29tbWVudDoz2",
                        content:
                          "<p>Again is there anyone who loves or pursues or desires to obtain paiits ecause it is pain, but because occasionally circumstances occur in which</p>\n",
                        date: "2023-02-20 07:48:41",
                      },
                    ],
                  },
                },
              ],
            },
            featuredImage: {
              node: {
                mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                  Math.floor(Math.random() * 8) + 1
                }.jpg`,
              },
            },
            galleryImages: {
              nodes: [
                {
                  id: "cG9zdDoxODY=",
                  databaseId: 186,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxODg=",
                  databaseId: 188,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTA=",
                  databaseId: 190,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTI=",
                  databaseId: 192,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
                {
                  id: "cG9zdDoxOTQ=",
                  databaseId: 194,
                  mediaItemUrl: `https://klbtheme.com/chakta/wp-content/uploads/2021/01/products-${
                    Math.floor(Math.random() * 8) + 1
                  }-600x600.jpg`,
                },
              ],
            },
            id: "cHJvZHVjdDoxNzU=",
            name: "Titanium Wheel Cover",
            averageRating: 4,
            shortDescription:
              "<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium dolorem udantium, totam rem aperiam, eaque ipsa quae abventore veritatis et quasi architecto beatae vitae dicta sunt explicabomo enim ipsam</p>\n",
            description:
              "<p>Sorem ipsum dolor sit amet consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur xcepteur sint occaecat cupidatat non proident, sunt.</p>\n<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cdolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudinInteger in mauris eu nibh euismod gravida. Duis ac tellus et risus vulputate vehicula. Donec lobortis risus a elit. Etiam tempor. Ut ullamcorper</p>\n",
            databaseId: 175,
            productId: 175,
            regularPrice: "$150.00",
            salePrice: "$120.00",
            slug: "titanium-wheel-cover",
          },
        },
      ],
    },
  },
  extensions: {
    debug: [],
    queryAnalyzer: {
      keys: "b7f0872b98303d2b1d28045ef711767459ec60c228ced3e74bd805cf027340a graphql:Query operation:MyQuery7 list:variableproduct list:externalproduct list:groupproduct list:simpleproduct list:menu list:menuitem list:tag list:category list:comment list:productcategory list:post list:page list:mediaitem",
      keysLength: 292,
      keysCount: 16,
      skippedKeys: "",
      skippedKeysSize: 0,
      skippedKeysCount: 0,
      skippedTypes: [],
    },
  },
};

export default product;
