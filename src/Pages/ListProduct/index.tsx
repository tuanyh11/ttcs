...
import { deleteProduct, duplicateProduct, getProducts } from "../../api/product";
                ...
                <tr key={product._id} className="intro-x">
                  <td className="px-5 py-3 dark:border-darkmode-300 first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                    <div className="flex">
                      ...
                      return <div key={index} className={`w-10 h-10 image-fit zoom-in ${index !== 0 ? '-ml-5': ''}`}>
                        <img
                  ...
                  <td className="px-5 py-3 dark:border-darkmode-300 first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                    <div className={`flex items-center justify-center  ${product.isActive ? ' text-green-600': 'text-danger'}`}>
                      <RectagMark/>
                  ...
                  <td className="px-5 dark:border-darkmode-300 first:rounded-l-md last:rounded-r-md  bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] py-0 relative before:block before:w-px before:h-8 before:bg-slate-200 before:absolute before:left-0 before:inset-y-0 before:my-auto before:dark:bg-darkmode-400">
                    <div className="flex items-center justify-center">
                      <Link className="flex items-center mr-3" to={`/e-commerce/edit-product/${product._id}`}>
                       <RectagMark/>
                        Edit
                      </Link>
                      <button onClick={() => handleOnDelete(product._id as string)} className="flex items-center mr-3 text-danger" >
                        <Trash/>
                        Delete
                      </button>
                      <button onClick={() => handleOnDuplicate(product._id as string)} className="flex items-center text-green-600" >
                        <DuplicateIcon/>
                        Duplicate
                      </button>
                    </div>
